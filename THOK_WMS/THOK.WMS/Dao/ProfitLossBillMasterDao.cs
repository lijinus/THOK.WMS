...
{
    public class ProfitLossBillMasterDao:BaseDao
    {
        ...
        public void DeleteEntity(DataSet dataSet)
        {
            try
            {
                foreach (DataRow dataRow in dataSet.Tables[0].Rows)
                {
                    ...
                    {
                        ExecuteNonQuery("delete WMS_PL_BILLDETAIL WHERE BILLNO='" + dataRow["BILLNO", DataRowVersion.Original] + "'");
                        ExecuteNonQuery("delete WMS_PL_BILLMASTER WHERE ID='" + dataRow["ID", DataRowVersion.Original] + "'");
                        ExecuteNonQuery("update WMS_CHECK_BILLMASTER SET STATUS='4' WHERE BILLNO='" + dataRow["CHECKBILLNO", DataRowVersion.Original] + "'");
                    }
                }
            }
            catch (Exception ex)
