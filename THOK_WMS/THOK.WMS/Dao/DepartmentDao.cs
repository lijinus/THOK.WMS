...
namespace THOK.WMS.Dao
{
    public class DepartmentDao:BaseDao
    {
        ...
        {
            string sql = "select top 1 DEPTCODE from BI_DEPARTMENT order by DeptCode desc";
            string sn = (string)ExecuteScalar(sql);
            DataSet ds= new DataSet();
            ds.ReadXml(AppDomain.CurrentDomain.BaseDirectory + "code\\TableXML\\BI_DEPARTMENT.xml");
        ...
        public void DeleteEntity(DataSet dataSet)
        {
            try
            {
                foreach (DataRow dataRow in dataSet.Tables[0].Rows)
                {
                    ...
                    {
                        ExecuteNonQuery("delete BI_DEPARTMENT WHERE DEPTCODE='" + dataRow["DEPTCODE", DataRowVersion.Original] + "'");
                    }
                }
            }
            catch (Exception ex)
