...
{
    public class EntryBillMasterDao : BaseDao
    {
        ...
        public void DeleteEntity(DataSet dataSet)
        {
            try
            {
                foreach (DataRow dataRow in dataSet.Tables[0].Rows)
                {
                    ...
                    {
                        ExecuteNonQuery("delete WMS_IN_BILLDETAIL WHERE BILLNO='" + dataRow["BILLNO", DataRowVersion.Original] + "'");
                        ExecuteNonQuery("delete WMS_IN_BILLMASTER WHERE ID='" + dataRow["ID", DataRowVersion.Original] + "'");
                    }
                }
            }
            catch (Exception ex)
