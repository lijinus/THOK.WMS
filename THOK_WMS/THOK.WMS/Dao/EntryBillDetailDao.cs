...
{
    public class EntryBillDetailDao : BaseDao
    {
        ...
        public void DeleteEntity(DataSet dataSet)
        {
            try
            {
                foreach (DataRow dataRow in dataSet.Tables[0].Rows)
                {
                    ...
                    {
                        ExecuteNonQuery("delete WMS_IN_BILLDetail WHERE ID='" + dataRow["ID", DataRowVersion.Original] + "'");
                        ExecuteNonQuery("delete DWV_IWMS_IN_STORE_BILL_DETAIL WHERE STORE_BILL_DETAIL_ID='" + dataRow["ID", DataRowVersion.Original] + "'");
                    }
                }
            }
            catch (Exception ex)
