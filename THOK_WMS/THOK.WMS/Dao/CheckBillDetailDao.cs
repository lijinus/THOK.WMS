...
{
    public class CheckBillDetailDao : BaseDao
    {
        ...
        public void BatchInsertDetail(DataTable tableDetail)
        {
            BatchInsert(tableDetail, "WMS_CHECK_BILLDETAIL");
        }
        ...
        public void DeleteEntity(DataSet dataSet)
        {
            try
            {
                foreach (DataRow dataRow in dataSet.Tables[0].Rows)
                {
                    ...
                    {
                        ExecuteNonQuery("delete WMS_CHECK_BILLDetail WHERE ID='" + dataRow["ID", DataRowVersion.Original] + "'");
                    }
                }
            }
            catch (Exception ex)
